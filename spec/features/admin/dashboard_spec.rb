...
RSpec.describe "admin dashboard" do
  describe 'happy path' do
    before :each do
      @admin_user = User.create!(name: "Bob the Admin", email: "adminbob@email.com", role: 1)
    ...
    it 'can see all users' do 
      ...
      within "#users-#{@user_1}" do
        expect(page).to have_content(@user_1.name)
        expect(page).to have_content(@user_1.email)
      end

      within "#users-#{@user_2}" do
        expect(page).to have_content(@user_2.name)
        expect(page).to have_content(@user_2.email)
      end

      within "#users-#{@user_3}" do
        expect(page).to have_content(@user_3.name)
        expect(page).to have_content(@user_3.email)
      end

      within "#users-#{@user_4}" do
        expect(page).to have_content(@user_4.name)
        expect(page).to have_content(@user_4.email)
      end

      within "#users-#{@user_5}" do
